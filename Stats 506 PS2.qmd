...
monte_carlo_simulation <- function(num_trials, num_dice) {
  profits <- numeric(num_trials) # Create a vector to store profits from all trials
...
cat('Average profit after 10,000 Monte Carlo simulations with 1,000 dice: ', monte_carlo_simulation(10000, 1000))
```
No, this is not a fair game for the game operator because the average profit after 10,000 Monte Carlo simulations using 1,000 dice each game is $669.99 which indicates that, on average, the player can expect to win a significant amount of money over the course of many games. The game operator will lose more and more money the more times the game is played and thus is not fair and is more favorable towards the player.
...
```
This distribution does look a lot less skewed than the untransformed variable, so I will use the log-transformation of highway mpg going forward.
